...
definePageMeta({
    name: 'complianceCommitmentSection_id',
...
const id = route.params.complianceCommitmentSection_id;
...
onMounted(async () => {
    isClient.value = true;
    console.log('Compliance Commitment section editor mounted');
...
const populateData = (newData) => {
    ...
    formData.buttons = (newData.buttons && Array.isArray(newData.buttons) && newData.buttons.length)
        ? newData.buttons
        : [{
            button_name: '',
            link: '',
...
const fetchRecords = async () => {
    try {
        console.log('Fetching Compliance Commitment section data for ID:', id);
        const res = await nuxtApp.$axios.get(`/cms/page-section/${id}`);
        const record = res.data?.record || res.data;

        if (!record || !record.id) {
            console.error('No valid section record found for Compliance Commitment:', res.data);
            nuxtApp.$toast.error('Section data not found.');
            return;
        }

        sectionData.value = record;
        console.log('Compliance Commitment section data set:', sectionData.value);
    } catch (err) {
