...
const id = route.params.websiteUsersSection_id;
...
onMounted(async () => {
    isClient.value = true;
    console.log('Website Users section editor mounted');
...
const fetchRecords = async () => {
    try {
        console.log('Fetching Website Users section data for ID:', id);
        const res = await nuxtApp.$axios.get(`/cms/page-section/${id}`);
        const record = res.data?.record || res.data;

        if (!record || !record.id) {
            console.error('No valid section record found for Website Users:', res.data);
            nuxtApp.$toast.error('Section data not found.');
            return;
        }

        sectionData.value = record;
        console.log('Website Users section data set:', sectionData.value);
    } catch (err) {
