<template>
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit"
            ...
            <!-- Availment Instructions: PDF upload (multiple PDFs) -->
            <div v-if="isAvailmentInstructionsSection" class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">PDF Files</h2>
                <p class="text-sm text-black/60">
                    Upload PDF files for Availment Instructions (e.g., guides, forms). You can upload more than one.
                </p>
                <ImageHandler
                    label="PDF Files"
                    :data="pdfFiles"
...
definePageMeta({
    name: 'availmentProcedureSection_id',
...
const id = route.params.availmentProcedureSection_id;
...
// Availment Instructions: enable multi-PDF uploader
const isAvailmentInstructionsSection = computed(() => {
    const name = sectionData.value?.name?.toLowerCase() || '';
    return name.includes('availment instructions');
});
...
const submit = async () => {
    ...
    // Append PDF file(s) for Availment Instructions sections
        ...
        try {
            await nuxtApp.$axios
                ...
                .then(response => {
                    ...
                    if (isAvailmentInstructionsSection.value && formData.pdf?.length) {
                        formData.pdf = [];
                    }
