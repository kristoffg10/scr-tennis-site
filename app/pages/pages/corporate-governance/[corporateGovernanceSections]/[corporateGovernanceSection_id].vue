    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                />
                <TextField v-if="sectionData && sectionData.sub_text !== undefined && !isAnnualIntegratedReportFile" v-model="formData.sub_text" name="sub_text" label="Sub Text" placeholder="Enter sub text" :rules="'max:255'" :hasErrors="hasErrors" />
                
                <!-- Section Cards Module Links (for Content Block) -->
                <HowToAdd
                    v-if="sectionData && (sectionData.cards !== undefined || sectionData.name === 'Content Block')"
                    title="Content Block"
                    :link="`/section-cards/${sectionData.id}`" />
                
                <!-- Section Files Module Links (for Annual Integrated Report File) -->
                <HowToAdd
                    v-if="sectionData && (sectionData.files !== undefined || sectionData.name === 'Annual Integrated Report File')"
                    title="Files"
...
const formData = reactive({ title: '', description: '', sub_text: '', has_button: 0, buttons: [], main_image: [], mobile_image: [], file: [] });
...
const isAnnualIntegratedReportFile = computed(() => sectionData.value?.name?.toLowerCase() === 'annual integrated report file');
...
const submit = async () => {
    ...
    if (formData.file?.length) formData.file.forEach(f => form_data.append('file[]', f));
