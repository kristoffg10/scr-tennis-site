...
const id = route.params.applicantsSection_id;
...
onMounted(async () => {
    isClient.value = true;
    console.log('Applicants section editor mounted');
...
const fetchRecords = async () => {
    try {
        console.log('Fetching Applicants section data for ID:', id);
        const res = await nuxtApp.$axios.get(`/cms/page-section/${id}`);
        const record = res.data?.record || res.data;

        if (!record || !record.id) {
            console.error('No valid section record found for Applicants:', res.data);
            nuxtApp.$toast.error('Section data not found.');
            return;
        }

        sectionData.value = record;
        console.log('Applicants section data set:', sectionData.value);
    } catch (err) {
