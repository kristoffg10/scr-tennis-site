...
definePageMeta({
    name: 'legalDisclaimerSection_id',
...
const id = route.params.legalDisclaimerSection_id;
...
onMounted(async () => {
    isClient.value = true;
    console.log('Legal Disclaimer section editor mounted');
...
const fetchRecords = async () => {
    try {
        console.log('Fetching Legal Disclaimer section data for ID:', id);
        const res = await nuxtApp.$axios.get(`/cms/page-section/${id}`);
        const record = res.data?.record || res.data;

        if (!record || !record.id) {
            console.error('No valid section record found for Legal Disclaimer:', res.data);
            nuxtApp.$toast.error('Section data not found.');
            return;
        }

        sectionData.value = record;
        console.log('Legal Disclaimer section data set:', sectionData.value);
    } catch (err) {
