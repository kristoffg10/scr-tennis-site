    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        ...
        <Form v-else id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <!-- Section Tabs Module Links (for FAQ Topics - tabs with FAQs) -->
                <HowToAdd
                    v-if="sectionData && (sectionData.tabs !== undefined || sectionData.name === 'FAQ Topics')"
                    title="FAQ Topics"
                    :link="`/section-tabs/${sectionData.id}`" />
                
                <!-- Note: Each tab in FAQ Topics will have FAQs accessible via /section-tabs/{parent_id}/{tab_id}/section-faqs -->
            </div>
...
<script setup>
    ...
    const id = route.params.helpFaqCenterSection_id;
