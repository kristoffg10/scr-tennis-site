    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <!-- Requirement Checklist File Cards Module Links -->
                <HowToAdd
                    v-if="sectionData && sectionData.name === 'Requirement Checklist File'"
                    title="Requirement Checklist Instructions"
                    :link="`/section-cards/${sectionData.id}`"
                    button-label="Add Instructions" />

                <!-- Requirement Checklist File Module Links -->
                <HowToAdd
                    v-if="sectionData && sectionData.name === 'Requirement Checklist File'"
                    title="Requirement Checklist File"
                    :link="`/section-files/${sectionData.id}`" />
            </div>
...
definePageMeta({
    name: 'hospitalAccreditationSection_id',
...
const id = route.params.hospitalAccreditationSection_id;
...
const submit = async () => {
    ...
    // Read title and sub_text from the form so updates persist after refresh
