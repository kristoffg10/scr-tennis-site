    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <Quill v-if="(isBannerSection || isCTASection || isTestimonialsSection || isFAQsSection) && isClient && !isMobileAppCTASection && !isTelemedicineFaqsSection" 
                ...
                <TextField v-if="sectionData && sectionData.sub_text !== undefined && !isMobileAppCTASection" v-model="formData.sub_text" name="sub_text" 
                ...
                <!-- Section Hotline Module Links -->
                <HowToAdd
                    v-if="sectionData && sectionData.name === 'Call a doc hotline'"
                    title="Call a doc hotline"
                    :link="`/section-hotline/${sectionData.id}`" />
            </div>
            ...
            <!-- Buttons Section - For Banner and CTA sections (hidden for Telemedicine banner and Mobile App CTA) -->
            <div v-if="(isBannerSection || isCTASection) && !isTelemedicineBannerSection && !isMobileAppCTASection"
                class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
...
const id = route.params.telemedicineSection_id;
...
// Telemedicine FAQs section where we should hide description field
const isTelemedicineFaqsSection = computed(() => {
    return route.params.telemedicineSections === 'faqs';
});

// Telemedicine banner where we should hide buttons
const isTelemedicineBannerSection = computed(() => {
    return sectionData.value && sectionData.value.name &&
        sectionData.value.name.toLowerCase() === 'banner';
});

// Telemedicine Mobile App CTA section where we should hide description, sub text, and buttons
const isMobileAppCTASection = computed(() => {
    const name = sectionData.value && sectionData.value.name
        ? sectionData.value.name.toLowerCase()
        : '';
    const slug = secondLastSegment.value ? secondLastSegment.value.toLowerCase() : '';
    return name.includes('mobile app cta') || slug === 'mobile-app-cta';
});
