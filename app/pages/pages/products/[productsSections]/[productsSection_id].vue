    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <Quill v-if="shouldShowDescriptionSection && isClient" 
            ...
            <!-- Images Section - For Banner, CTA, and specific brochure CTA sections -->
            <div v-if="(isBannerSection || isCTASection || isBrochureCTASection) && !isProductsRootBannerSection" 
                ...
                <hr v-if="isBannerSection || isCTASection || isBrochureCTASection" class="h-[1px] w-full border-gray" />
                <ImageHandler v-if="isBannerSection || isCTASection || isBrochureCTASection" label="Mobile Image" max="1" size="5"
                    :data="mobile_image"
            ...
            <!-- Buttons Section - For Banner and CTA sections (hidden for specific CTAs) -->
            <div v-if="(isBannerSection || isCTASection) && !isNoButtonsSection"
                class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
...
const id = route.params.productsSection_id;
...
// Voluntary Employee Benefit Program - Brochure CTA: treat as CTA for images
const isBrochureCTASection = computed(() => {
    return route.params.productsSections === 'brochure-cta';
});
...
// Root Products Banner: hide description, images, and buttons section
const isProductsRootBannerSection = computed(() => {
    return route.params.productsSections === 'banner';
});

// Product sections where description and buttons should be hidden
const noButtonsOrDescriptionSections = [
...
const isNoButtonsOrDescriptionSection = computed(() => {
    if (isProductsRootBannerSection.value) return true;
    if (!sectionData.value || !sectionData.value.name) return false;
    const name = sectionData.value.name.toLowerCase();
    return noButtonsOrDescriptionSections.some((key) => name.includes(key));
});

// Product sections where buttons should be hidden (e.g. Request Proposal CTA, Annual Report CTA, Group/Individual Plan CTA, root Products Banner)
const isNoButtonsSection = computed(() => {
    return isNoButtonsOrDescriptionSection.value;
});

// Sections where the rich-text Description field should be shown
const shouldShowDescriptionSection = computed(() => {
    if (isNoButtonsOrDescriptionSection.value) return false;
    return (
        isBannerSection.value ||
        isCTASection.value ||
        isTestimonialsSection.value ||
        isFAQsSection.value
    );
});
...
const submit = async () => {
    ...
    // Align sub_text handling with Homepage implementation:
    // read from the actual input element, then replace any existing FormData entry
