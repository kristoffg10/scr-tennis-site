...
definePageMeta({
    name: 'groupInsurancePlanSection_id',
...
const id = route.params.groupInsurancePlanSection_id;
...
// Group Insurance Plan section where buttons should be hidden
const isRequestProposalCTASection = computed(() => {
    return sectionData.value && sectionData.value.name &&
        sectionData.value.name.toLowerCase().includes('request proposal cta');
});
