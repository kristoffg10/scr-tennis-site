    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        <Form v-if="sectionData && formattedTitle" id="form" @submit="submit()" @invalid-submit="onInvalidSubmit"
            ...
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <Quill v-if="(isBannerSection || isCTASection || isTestimonialsSection || isFAQsSection) && !isBrochureCta && !isVEBPFaqsSection && isClient" 
                ...
                <TextField v-if="sectionData && sectionData.sub_text !== undefined && !isBrochureCta && !isVEBPBannerSection" v-model="formData.sub_text" name="sub_text" 
                    label="Sub Text" placeholder="Enter sub text" :rules="'max:255'" :hasErrors="hasErrors" />

                <!-- Section Cards Module Links (hidden for Brochure CTA) -->
                <HowToAdd
                    v-if="sectionData && !isBrochureCta && (sectionData.name === 'Availment Instructions' || sectionData.name === 'Enrollment Guide' || sectionData.name === 'Claim Instructions' || sectionData.name === 'Plan Highlights' || sectionData.name === 'App Highlights' || sectionData.name === 'Telemedicine Features' || sectionData.name === 'Benefits' || sectionData.name === 'Optional Benefits')"
                    :title="sectionData.name"
                    :link="`/section-cards/${sectionData.id}`" />
                
                <!-- Section Tabs Module Links -->
                <HowToAdd
                    v-if="sectionData && !isBrochureCta && sectionData.name === 'Claim Requirements'"
                    title="Claim Requirements"
                    :link="`/section-tabs/${sectionData.id}`" />
                
                <!-- Section Videos Module Links -->
                <HowToAdd
                    v-if="sectionData && !isBrochureCta && sectionData.name === 'Video'"
                    title="Video"
                    :link="`/section-videos/${sectionData.id}`" />
            </div>

            <!-- Images Section - For Banner, CTA and Brochure CTA -->
            <div v-if="isBannerSection || (isCTASection && sectionData.has_button) || isBrochureCta" 
                class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">
                    {{ isBrochureCta ? 'Desktop Image' : `${formattedTitle} Image` }}
                </h2>
                <ImageHandler label="Desktop Image" :data="main_image" max="1" size="5"
                ...
                <hr v-if="isBannerSection || isBrochureCta" class="h-[1px] w-full border-gray" />
                <ImageHandler v-if="isBannerSection || isBrochureCta" label="Mobile Image" max="1" size="5"
                    :data="mobile_image"
            ...
            <!-- PDF Upload - Brochure CTA only (max 1) -->
            <div v-if="isBrochureCta && sectionData" class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">PDF File</h2>
                <p class="text-sm text-black/60">Upload brochure PDF (max 1 file)</p>
                <ImageHandler
                    label="PDF File"
                    max="1"
                    size="10"
            ...
            <!-- Buttons Section - For Banner and CTA sections (hidden for VEBP banner and Brochure CTA) -->
            <div v-if="(isBannerSection || isCTASection) && !isVEBPBannerSection && !isBrochureCta"
                class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
...
definePageMeta({
    name: 'voluntaryEmployeeBenefitProgramSection_id',
...
const id = route.params.voluntaryEmployeeBenefitProgramSection_id;
...
const isBrochureCta = computed(() => {
    const name = sectionData.value?.name?.toLowerCase() || '';
    const path = route.path.toLowerCase();
    return (name.includes('brochure cta') || path.includes('brochure-cta'));
});
...
// VEBP FAQs section where we should hide description field
const isVEBPFaqsSection = computed(() => {
    return route.params.voluntaryEmployeeBenefitProgramSections === 'faqs';
});

// VEBP banner where we should hide buttons
const isVEBPBannerSection = computed(() => {
    return sectionData.value && sectionData.value.name &&
        sectionData.value.name.toLowerCase() === 'banner';
});
...
const pdfFiles = computed(() => {
    ...
    const files = sectionData.value.files || [];
    return files.filter(file => (file.category || '').toLowerCase() === 'pdf' || (file.name || file.path || '').toLowerCase().endsWith('.pdf'));
});
