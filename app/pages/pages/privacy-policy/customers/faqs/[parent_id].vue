        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link
                        :to="`/section-faqs/${parentId}/create`"
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th
                            class="px-[24px] py-[8px] flex gap-[8px] cursor-pointer"
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            ...
                            <td class="px-[24px] py-[8px]">
                                ...
                                <span class="text-sm text-black">
                                    {{ $moment(faq.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                >
                                    <router-link
                                        :to="`/section-faqs/${parentId}/${faq.id}`"
                                    ...
                                    <button
                                        ...
                                        @click="openDeletePopup(`/cms/section-faq/${parentId}/${faq.id}`)"
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="20"
...
onMounted(async () => {
    pagination.reset();
    await fetchRecords();
    pageTitle.setTitle('Section FAQs');
    pageTitle.setBreadcrumbs(['Pages', 'Section FAQs']);
...
const fetchRecords = async () => {
    try {
        const faqs_response = await nuxtApp.$axios.get(
            `/cms/section-faq/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`,
        );
