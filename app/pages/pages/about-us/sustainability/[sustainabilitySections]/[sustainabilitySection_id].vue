    ...
    <div class="relative flex flex-col justify-start p-[16px] mb-[32px] w-full gap-y-[16px] overflow-y-auto">
        ...
        <Form v-else id="form" @submit="submit()" @invalid-submit="onInvalidSubmit" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                />
                <!-- <TextField
                    v-if="route.params.sustainabilitySections === 'cta' && sectionData && sectionData.sub_text !== undefined"
                    v-model="formData.sub_text"
                    name="sub_text"
                    :label="`${formattedTitle || sectionData?.name || 'Section'} Subtext`"
                    placeholder="Enter subtext"
                    :rules="'max:255'"
                    :hasErrors="hasErrors"
                /> -->
                <Quill
                    v-if="sectionData && (sectionData.order === 2 || sectionData.order === 3 || sectionData.order === 4) && isClient && route.params.sustainabilitySections !== 'cta'"
                ...
                <!-- Section Cards Module Links (for Content Block, excluded on Content Block editor) -->
                <HowToAdd
                    v-if="sectionData && sectionData.cards !== undefined && sectionData.name !== 'Content Block'"
                    title="Content Block"
                    :link="`/section-cards/${sectionData.id}`" />
            </div>
            <div v-if="sectionData && ((sectionData.order === 1 || sectionData.order === 3 || sectionData.order === 4) || route.params.sustainabilitySections === 'content-block')"  class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <ImageHandler 
                    ...
                    :required="sectionData.order === 1"
                    ref="imageHandler0"
                        v-model="formData.main_image[0]"
                    @update:file="handleFileUpdate('main_image', $event)"
                />
                <hr v-if="sectionData.order === 1" class="h-[1px] w-full border-gray">
                <ImageHandler v-if="sectionData.order === 1"
                    label="Mobile Image" 
                    max="1"
                    size="5"
                    :data="mobile_image"
            ...
            <!-- <div v-if="sectionData && (sectionData.name?.toLowerCase().includes('cta') || sectionData.has_button)" class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">{{ formattedTitle || sectionData?.name || 'Section' }} Button</h2>
                <div v-if="route.params.sustainabilitySections !== 'cta'" class="flex flex-col gap-[8px]">
                ...
                <div v-if="route.params.sustainabilitySections === 'cta' || formData.has_button" v-for="(button, index) in formData.buttons" :key="index" class="">
                    <Button
                        :name="`button`"
                        :button="button"
                        :showLink="false"
                        :label="route.params.sustainabilitySections === 'cta' ? 'Button Link' : 'Button Label'"
                    />
                </div>
            </div> -->
...
<script setup>
    ...
    const id = route.params.sustainabilitySection_id;
    ...
    const submit = async () => {
        ...
        form_data.set('has_button', (route.params.sustainabilitySections === 'cta' ? 1 : (formData.has_button ? 1 : 0))); 
        // For Content Block, rely on the textarea's current value.
        // For other sections (using Quill), explicitly send the reactive description.
        form_data.set('description', formData.description);
        // Handle optional sub_text similar to other page editors
    ...
    const populateData = (newData) => {
        if (newData) {
            ...
            formData.has_button = Number(newData.has_button) || route.params.sustainabilitySections === 'cta' ? 1 : 0
