        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/section-benefits/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        <th class="px-[24px] py-[8px] cursor-pointer flex gap-[8px]" @click="toggleSort('sequence')"> Seq.
                            <svg
                ...
                >
                    <template #item="{ element: card, index }">
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            ...
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ card.description ? (card.description.length > 50 ? card.description.substring(0, 50) + '...' : card.description) : '-' }}</td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(card.updated_at).format('MMM DD, YYYY') }}<br>
                            ...
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    <router-link
                                        :to="`/section-benefits/${parentId}/${card.id}`"
                                    ...
                                    <button
                                        ...
                                        @click="openDeletePopup(`/cms/section-benefit/${parentId}/${card.id}`)">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="18"
                                            height="20"
...
<script setup>
    ...
    const fetchRecords = async () => {
        try {
            const cards_response = await nuxtApp.$axios.get(`/cms/section-benefit/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
