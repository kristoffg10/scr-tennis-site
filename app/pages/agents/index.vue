    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="agents" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ agents.from ?? 0 }} - {{ agents.to ?? 0 }} 
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/agents/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">License Type</th>
                        <th class="px-[24px] py-[8px]">License ID</th>
                        <th class="px-[24px] py-[8px]">Effectivity</th>
                        <th class="px-[24px] py-[8px]">Expiry</th>
                ...
                <tbody>
                    <tr v-if="agents && agentsData.length === 0" class="h-[60px] bg-white">
                        <td colspan="7" class="px-[24px] py-[16px] text-center text-black/60">No agents found.</td>
                    </tr>
                    <template v-if="agents && agentsData.length" v-for="(agent, index) in agentsData" :key="agent.id">
                        <tr class="h-3"> <!-- Adjust the height as needed -->
                            <td colspan="7"></td> <!-- Empty cell to occupy space -->
                        </tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                {{ agent.lastname }}, {{ agent.firstname }} {{ agent.middle_initial ? agent.middle_initial + '.' : '' }}
                            </td>     
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ agent.license_type || '-' }}</td> 
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                <span class="inline-flex items-center gap-[6px]">
                                    {{ agent.license_id || '-' }}
                                    <button
                                        v-if="agent.license_id"
                                        type="button"
                                        class="inline-flex items-center justify-center w-[28px] h-[28px] rounded-full text-black/50 hover:bg-ui-hover hover:text-ui-color transition-colors"
                                        :title="`Copy License ID: ${agent.license_id}`"
                                        @click="copyLicenseId(agent.license_id)"
                                    >
                            ...
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                {{ agent.effectivity ? $moment(agent.effectivity).format('MMM DD, YYYY') : '-' }}
                            </td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                {{ agent.expiry ? $moment(agent.expiry).format('MMM DD, YYYY') : '-' }}
                            </td>
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(agent.updated_at ? agent.updated_at : agent.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(agent.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    <router-link
                                        ...
                                        :title="`Edit ${agent.lastname}, ${agent.firstname}`"
                                    >
                                        <svg
                                            width="18"
                                            height="18"
                                    ...
                                    <button
                                        ...
                                        :title="`Delete ${agent.lastname}, ${agent.firstname}`"
                                        @click="
                                            openDeletePopup(
                                                `/cms/agents/${agent.id}`
                                            )
...
<script setup>
    ...
    // Normalize: API returns { records: { data: [], from, to, total, last_page } } or { records: [] } when all=1
    const agentsData = computed(() => {
        if (!agents.value) return [];
        if (Array.isArray(agents.value)) return agents.value;
        if (agents.value && Array.isArray(agents.value.data)) return agents.value.data;
        return [];
    });
    ...
    onMounted( async () => {
        ...
        pageTitle.setBreadcrumbs(['Agents', 'Agents List']);
    ...
    const fetchRecords = async () => {
        try {
            const response = await nuxtApp.$axios.get(`/cms/agents?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value || ''}`); 
            const rec = response.data?.records;
            // Paginator: { data, from, to, total, last_page }; or array when all=1
            if (Array.isArray(rec)) {
                agents.value = { data: rec, from: 1, to: rec.length, total: rec.length, last_page: 1 };
            } else if (rec && typeof rec === 'object' && Array.isArray(rec.data)) {
                agents.value = rec;
            } else {
                agents.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
            }
            if (agents.value?.last_page) {
                pagination.setTotalPages(agents.value.last_page);
            }
        } catch (error) {
            console.error('Agents fetch error:', error);
            agents.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
            nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load agents. Check the API and try again.');
        }
    };
    ...
    const copyLicenseId = async (text) => {
        if (!text) return;
        try {
            await navigator.clipboard.writeText(text);
            nuxtApp.$toast.success('License ID copied to clipboard');
        } catch (err) {
            nuxtApp.$toast.error('Failed to copy');
        }
    };
