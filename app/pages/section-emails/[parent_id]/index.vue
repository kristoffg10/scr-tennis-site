    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="emails && emails.data" class="text-black font-medium text-base">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/section-emails/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                ...
                <tbody>
                    <template v-if="emails && emails.data" v-for="(email, index) in emails.data" :key="email.id">
                        <tr class="h-3"><td colspan="5"></td></tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ email.sequence }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ email.title || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ email.email || '-' }}</td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(email.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(email.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                <div v-if="activeMenuIndex === index" class="absolute bottom-0 left-0 bg-white flex flex-col p-[10px] gap-[10px] rounded-[10px] translate-y-[70%] font-[600] border border-gray z-50">
                                    <router-link :to="`/section-emails/${parentId}/${email.id}`" class="flex items-center gap-[10px] pb-[10px] border-b border-black/30">
                                    ...
                                    <button class="flex items-center gap-[10px] text-[#F12222]" @click="openDeletePopup(`/cms/section-email/${parentId}/${email.id}`)">
...
<script setup>
    ...
    onMounted( async () => {
        pagination.reset();
        await fetchRecords();
        pageTitle.setTitle("Section Emails");
        pageTitle.setBreadcrumbs(['Pages', 'Section Emails']);
    ...
    const fetchRecords = async () => {
        try {
            const emails_response = await nuxtApp.$axios.get(`/cms/section-email/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            emails.value = emails_response.data.records;
            pagination.setTotalPages(emails_response.data.records.last_page);
        } catch (error) {
