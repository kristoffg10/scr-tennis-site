        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/section-testimonials/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Company</th>
                ...
                <Draggable
                    v-if="testimonials && testimonials.data"
                    v-model="testimonials.data"
                    item-key="id"
                    handle=".drag-handle"
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-normal">
                                <div class="flex items-center gap-[8px]">
                                    ...
                                    <span>{{ testimonial.sequence }}</span>
                                </div>
                            </td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ testimonial.name || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ testimonial.position || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ testimonial.company || '-' }}</td>

                            <td class="px-[24px] py-[8px]">
                                {{ $moment(testimonial.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(testimonial.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                <div v-if="activeMenuIndex === index" class="absolute bottom-0 left-0 bg-white flex flex-col p-[10px] gap-[10px] rounded-[10px] translate-y-[70%] font-[600] border border-gray z-50">
                                    <router-link :to="`/section-testimonials/${parentId}/${testimonial.id}`" class="flex items-center gap-[10px] pb-[10px] border-b border-black/30">
                                    ...
                                    <button class="flex items-center gap-[10px] text-[#F12222]" @click="openDeletePopup(`/cms/section-testimonial/${parentId}/${testimonial.id}`)">
...
<script setup>
    ...
    onMounted( async () => {
        pagination.reset();
        await fetchRecords();
        pageTitle.setTitle("Section Testimonials");
        pageTitle.setBreadcrumbs(['Pages', 'Section Testimonials']);
    ...
    const fetchRecords = async () => {
        try {
            const testimonials_response = await nuxtApp.$axios.get(`/cms/section-testimonial/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
    ...
    const onReorder = async (newOrder) => {
        try {
            ...
            if (nuxtApp.$toast && nuxtApp.$toast.success) {
                nuxtApp.$toast.success('Testimonials reordered successfully!');
            }
        } catch (error) {
            console.error('Error reordering testimonials:', error);
            if (nuxtApp.$toast && nuxtApp.$toast.error) {
                nuxtApp.$toast.error('Failed to reorder testimonials.');
            }
        }
    };
