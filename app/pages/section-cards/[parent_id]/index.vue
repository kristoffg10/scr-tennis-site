    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="cards && cards.data" class="text-black font-medium text-base">
                ...
                <div v-if="!hideAddDelete" class="flex gap-[16px] items-center">
                    <router-link :to="`/section-cards/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        <th class="px-[24px] py-[8px]"> Seq.</th>
                ...
                >
                    <template #item="{ element: card, index }">
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            ...
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ card.title || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ descriptionPreview(card.description) }}</td>
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(card.updated_at ? card.updated_at : card.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">
                                    {{ $moment(card.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                ...
                                <div
                                    ...
                                    <router-link :to="`/section-cards/${parentId}/${card.id}`"
                                    ...
                                    <button
                                        ...
                                        @click="
                                            openDeletePopup(
                                                `/cms/section-card/${parentId}/${card.id}`
                                            )
                                        ">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
...
<script setup>
    ...
    // Section card parent IDs that hide Add and Delete (read-only list)
    const READ_ONLY_SECTION_CARD_IDS = ['98857472-68f4-4fe1-bb14-88ef07cc1f1e'];
    const hideAddDelete = computed(() => READ_ONLY_SECTION_CARD_IDS.includes(parentId));
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle("Section Cards");
        pageTitle.setBreadcrumbs(['Pages', 'Section Cards']);
    ...
    const fetchRecords = async () => {
        try {
            const cards_response = await nuxtApp.$axios.get(`/cms/section-card/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            cards.value = cards_response.data.records;

            pagination.setTotalPages(cards_response.data.records.last_page);
        } catch (error) {
    ...
    const descriptionPreview = (description, maxLen = 50) => {
    ...
    const onReorder = async (newOrder) => {
        try {
            ...
            if (nuxtApp.$toast && nuxtApp.$toast.success) {
                nuxtApp.$toast.success('Cards reordered successfully!');
            }
        } catch (error) {
            console.error('Error reordering cards:', error);
            if (nuxtApp.$toast && nuxtApp.$toast.error) {
                nuxtApp.$toast.error('Failed to reorder cards.');
            }
        }
    };
