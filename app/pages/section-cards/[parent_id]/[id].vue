        ...
        <Form v-if="cardData" id="form" @submit="submit()" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <template v-if="showImageUpload">
                    ...
                    :existing_images="cardData.images?.filter(img => img.category === 'mobile_image')"
...
const fetchRecords = async () => {
    try {
        const card_response = await nuxtApp.$axios.get(`/cms/section-card/${parentId}/${id}`);
...
const submit = async () => {
    ...
    try {
        const response = await nuxtApp.$axios.post(`/cms/section-card/${parentId}/${id}`, form_data, {
            headers: {
