    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full flex flex-col">
                <div class="flex gap-[16px] items-end">
                    ...
                    <div class="w-[200px]">
                        <label for="type" class="text-base font-medium text-black/70">
                            Filter by Type
                        </label>
                        <select 
                            id="type"
                            v-model="selectedType" 
                            class="w-full p-[16px] border border-[#2A2A2A]/20 rounded-[10px] bg-white outline-none focus:border-ui-color">
                            <option value="">All Types</option>
                            <option 
                                v-for="option in typeOptions.filter(opt => opt.value !== '')" 
                                :key="option.value" 
                                :value="option.value">
                                {{ option.label }}
                            </option>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="leaders" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ leaders.from ?? 0 }} - {{ leaders.to ?? 0 }} 
                    <span class="text-black/60">of</span> 
                    {{ leaders.total ?? 0 }} 
                    <span class="text-black/60">items</span>
                </p>
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/leaders/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Sequence</th>
                        <th class="px-[24px] py-[8px]">Date Edited</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="leaders && leadersData.length === 0" class="h-[60px] bg-white">
                        <td colspan="6" class="px-[24px] py-[16px] text-center text-black/60">No leaders found.</td>
                    </tr>
                    <template v-if="leaders && leadersData.length" v-for="(leader, index) in leadersData" :key="leader.id">
                        <tr class="h-3">
                            <td colspan="6"></td>
                        </tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ leader.name || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ leader.position || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                <span v-if="leader.type" class="inline-block px-3 py-1 rounded-full bg-ui-color text-white capitalize text-sm font-medium">{{ leader.type }}</span>
                                <span v-else class="inline-block px-3 py-1 rounded-full bg-ui-color text-white text-sm font-medium">-</span>
                            </td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ leader.sequence ?? '-' }}</td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(leader.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(leader.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    ...
                                    <button
                                        ...
                                        @click="openDeletePopup(`/cms/leaders/${leader.id}`)">
...
const leadersData = computed(() => {
    ...
    if (Array.isArray(leaders.value)) return leaders.value;
    if (leaders.value && Array.isArray(leaders.value.data)) return leaders.value.data;
    return [];
});
...
onMounted(async () => {
    pagination.reset();
    await fetchRecords();
    pageTitle.setTitle("Leaders");
    pageTitle.setBreadcrumbs(['Leaders', 'Leaders List']);
...
const fetchRecords = async () => {
    try {
        const typeParam = selectedType.value ? `&type=${selectedType.value}` : '';
        const response = await nuxtApp.$axios.get(`/cms/leaders?page=${pagination.page}&keyword=${keyword.value || ''}${typeParam}`);
        const rec = response.data?.records;
        // Paginator: { data, from, to, total, last_page }; or array when all=1
        if (Array.isArray(rec)) {
            leaders.value = { data: rec, from: 1, to: rec.length, total: rec.length, last_page: 1 };
        } else if (rec && typeof rec === 'object' && Array.isArray(rec.data)) {
            leaders.value = rec;
        } else {
            leaders.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        }
        if (leaders.value?.last_page) {
            pagination.setTotalPages(leaders.value.last_page);
        }
    } catch (error) {
        console.error('Leaders fetch error:', error);
        leaders.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load leaders. Check the API and try again.');
    }
};
