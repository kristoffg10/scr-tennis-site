    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="providers && providers.data" class="text-black font-medium text-base">
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/providers/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Specialization</th>
                        <th class="px-[24px] py-[8px]">Clinic</th>
                        <th class="px-[24px] py-[8px]">Location</th>
                ...
                <tbody>
                    <template v-if="providers && providers.data" v-for="(provider, index) in providers.data" :key="provider.id">
                        <tr class="h-3"> <!-- Adjust the height as needed -->
                            <td colspan="6"></td> <!-- Empty cell to occupy space -->
                        </tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ provider.name || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ provider.specialization || '-' }}</td> 
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ provider.clinic || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ provider.location || '-' }}</td>
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(provider.updated_at ? provider.updated_at : provider.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(provider.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
...
<script setup>
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle("Providers");
        pageTitle.setBreadcrumbs(['Providers', 'Providers List']);
    ...
    const fetchRecords = async () => {
        try {
            const providers_response = await nuxtApp.$axios.get(`/cms/providers?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            providers.value = providers_response.data.records;

            pagination.setTotalPages(providers_response.data.records.last_page);
        } catch (error) {
