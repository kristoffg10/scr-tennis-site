<template>
    <div class="relative w-full p-[16px] bg-offwhite flex flex-col gap-[16px] overflow-y-auto mb-[32px]">
        <Form v-if="amenityData" id="form" @submit="submit()" class="w-full gap-[16px] flex flex-col">
    ...
    const id = route.params.amenity_id;
    ...
    onMounted(() => {
        ...
        pageTitle.setTitle(`Edit Amenity`);
        pageTitle.setBreadcrumbs(['Properties', property_type, propertyData.value ? propertyData.value.name : '', 'Amenities', 'Edit Amenity']);
        pageTitle.setPageFrom('Amenities');
        pageTitle.setPageFromRoute('/properties/' + propertyType + '/' + useRoute().params.id + '/amenities');
        fetchRecords();
        // populateData(sectionData.value);
    });
    
    const amenityData = ref(null);
    const fetchRecords = async () => {
        try {

            const amenities_response = await nuxtApp.$axios.get(`/cms/amenities/${id}`); 
            amenityData.value = amenities_response.data.record;

            populateData(amenityData.value);
        } catch (error) {
    ...
    const submit = async () => {
            ...
            try {
                const response = await nuxtApp.$axios.post(`/cms/amenities/${id}`, form_data , {
                    headers: {
                    ...
                    nuxtApp.$toast.success('Amenity updated successfully!');
                }); 
    ...
    const populateData = (data) => {
        formData.title = data.content;
        formData.enabled = Number(data.enabled)  ? true : false;
    }
    
    
    const { data: propertyData, error, status: isLoading } = useAsyncData(`amenity-${useRoute().params.id}`, async () => {
        try {
