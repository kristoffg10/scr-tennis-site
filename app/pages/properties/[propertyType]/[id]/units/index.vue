        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <!-- <button @click="openPopup"  type="button" class="px-[16px] py-[10px] rounded-[10px] border border-ui-color text-ui-color text-base font-medium transition
                    hover:bg-dark-ui-color hover:text-white">
                        Batch Import    
                    </button> -->
                    <router-link v-if="propertyData" :to="`/properties/${propertyType}/${propertyData.id}/units/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Unit Name</th>
                        <th class="px-[24px] py-[8px]">Unit Type</th>
                ...
                <tbody>
                    <template v-if="properties && properties.data" v-for="(property, index) in properties.data" :key="property.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            ...
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ property.name }}</td>
                            <td class="px-[24px] py-[8px] ]">{{ property.unit_type.name }}</td>
                            <td class="px-[24px] py-[8px] flex gap-[8px] items-center h-[64px]" :class="{'text-success' : property.enabled, 'text-orange' : !property.enabled}">
                                <svg xmlns="http://www.w3.org/2000/svg" width="7" height="8" viewBox="0 0 7 8" 
                                :class="{'fill-success' : property.enabled, 'fill-orange' : !property.enabled}">
                            ...
                            <td class="px-[24px] py-[8px]  relative w-[150px]">
                                ...
                                <div
                                    ...
                                    <router-link :to="`/properties/${propertyType}/${route.params.id}/units/${property.id}`"
...
<script setup>
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle(`Units List`);
        pageTitle.setBreadcrumbs(['Properties', property_type, await propertyData.value ? propertyData.value.name : '', 'Units List']);
    ...
    const fetchRecords = async () => {
        try {
            ...
            const properties_response = await nuxtApp.$axios.get(`/cms/units?page=${pagination.page}
...
const closePopup = () => {
    ...
    const onSubmit = async () => {
        ...
        try {
                const response = await nuxtApp.$axios.post(`/cms/batch-upload?query=units`, form_data , {
                    headers: {
                        'Content-Type': 'multipart/form-data',
                    },
                }).then((response) => {
                    const record = response.data.record;
                    nuxtApp.$toast.success('Units imported successfully!');
                    fetchRecords();
                    closePopup();
                }); 
