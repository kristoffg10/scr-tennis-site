    ...
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    Displaying {{ records.length }} of {{ totalRecords }} Job
                    Locations
                </div>
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.job_location_count}} Job Listings</span>
                        </td>
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.job_location_count === 0"
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Job Location' : 'Add a Job Location'">
            <Form
                @submit="onSubmit"
                ...
                <TextField
                    label="Job Location"
                    name="jobLocation"
                    v-model="formInitialValues.jobLocation"
                ...
                <div class="flex gap-[24px] mt-[16px] ml-auto max-w-max">
                    ...
                    <button
                       ...
                       {{isEditing ? 'Edit Job Location' : 'Add  Job Location'}}
                    </button>
                </div>
            </Form>
        </PopupForm>
...
onMounted(() => {
    pageTitle.reset();
    pageTitle.setTitle('Job Locations');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Careers', 'Job Locations']);
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/job-locations?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        console.log('doing');
...
const formInitialValues = computed(() => ({
    jobLocation: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Job Location updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Job Location added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the Job Location'
        );
    } finally {
