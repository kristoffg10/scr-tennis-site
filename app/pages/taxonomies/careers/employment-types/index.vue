<template>
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                ...
                <button
                    ...
                    <span> Add an Employment Type</span>
                </button>
            </div>
            <!-- Table  -->
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span>{{record.job_type_count}} Job Listings</span>
                        </td>
                        <td class="py-[34.5px] relative">
                            ...
                            <div
                                ...
                                <button v-if="record.job_type_count === 0"
        ...
        <PopupForm
            ...
            :title="
                isEditing ? 'Edit Employment Type' : 'Add an Employment Type'
            ">
            <Form
                @submit="onSubmit"
                ...
                <TextField
                    ...
                    placeholder="Enter an employment type name"
                    v-model="formInitialValues.employmentTypeName"
...
onMounted(() => {
    pageTitle.reset();
    pageTitle.setTitle('Employment Types');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Careers', 'Employment Types']);
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/employment-types?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
...
const formInitialValues = computed(() => ({
    employmentTypeName: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Employment type updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Employment type added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the Employment type'
        );
    } finally {
