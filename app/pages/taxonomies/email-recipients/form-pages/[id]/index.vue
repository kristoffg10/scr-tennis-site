<template>
        ...
        <div class="bg-white p-4 rounded-lg">
            ...
            <Form
                ...
                <div class="flex flex-col gap-4">
                    <!-- Submission Type (Title) - full width, readonly -->
                    <div class="flex flex-col gap-2">
                        <p class="font-medium">Submission Type (Title)</p>
                        <div class="readonly-field-wrapper">
                            <Field
                                readonly
                                class="readonly-field w-full p-4 border border-[#2A2A2A]/20 rounded-[10px] bg-gray-100"
                                rules="required"
                                v-model="formPage"
                                name="formPage"
                                placeholder="Enter submission type title" />
                        </div>
                        <ErrorMessage
                            name="formPage"
                    ...
                    <!-- Enter email - same width as title, Add Email button inline -->
                    <div class="flex flex-col gap-2">
                        <p class="font-medium">Enter email</p>
                        <div class="flex gap-2 w-full">
                            <Field
                                ...
                                class="flex-1 min-w-0 p-4 border border-[#2A2A2A]/20 placeholder:opacity-20 rounded-[10px]" />
                            <button
                                type="button"
                                @click="addEmail"
                                class="flex items-center gap-1 shrink-0 px-4 py-2 rounded-[10px] bg-ui-color text-white hover:opacity-90 transition-opacity">
                                <svg
    ...
    <PopupDelete
        ...
        :deletePath="`/cms/taxonomy/submission_type/${route.params.id}`" />
</template>
...
onMounted(async () => {
    pageTitle.reset();
    pageTitle.setTitle('Edit Email Recipients');
...
const addEmail = () => {
    if (currentEmail.value && isValidEmail(currentEmail.value)) {
        if (!record.value.email_recipients) {
            record.value.email_recipients = [];
        }
        if (!record.value.email_recipients.includes(currentEmail.value)) {
            record.value.email_recipients.push(currentEmail.value);
            currentEmail.value = '';
            errorMessage.value = '';
        } else {
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        await nuxtApp.$axios.patch(
            `/cms/taxonomy/submission_type/${route.params.id}`,
            {
                name: values.formPage,
                email_recipients: record.value.email_recipients || [],
            }
        );
        nuxtApp.$toast.success('Email recipients updated successfully!');
    } catch (error) {
...
const fetchRecords = async () => {
    try {
        ...
        record.value = response.data.record || response.data;
        record.value.email_recipients = record.value.email_recipients || [];
        formPage.value = record.value.name;
    } catch (error) {
        console.error('Error fetching record:', error);
        nuxtApp.$toast.error('Failed to load submission type');
    }
};
...
.readonly-field-wrapper :deep(input),
.readonly-field-wrapper :deep(.readonly-field) {
    outline: none !important;
    box-shadow: none !important;
}

.readonly-field-wrapper :deep(input:hover),
.readonly-field-wrapper :deep(input:focus),
.readonly-field-wrapper :deep(input:active) {
