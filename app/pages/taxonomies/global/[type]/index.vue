<template>
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full flex flex-wrap items-end gap-x-4">
                <div class="min-w-[180px]">
                    <label for="type" class="text-base font-medium text-black/70 block mb-1">Type</label>
                    <select id="type" v-model="selectedType" class="w-full border border-black/20 rounded-[10px] px-3 py-2 text-base">
                        <option v-for="opt in typeOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</option>
                    </select>
                </div>
                <div class="min-w-[200px] flex-1">
                    <label for="search" class="text-base font-medium text-black/70">Search by name</label>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/taxonomies/global/${typeParam}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition hover:bg-dark-ui-color flex gap-[4px]">
            ...
            <table class="min-w-full border-separate border-spacing-0 rounded-[10px] overflow-hidden bg-off-white relative z-10 pb-[140px]">
                ...
                <tbody>
                    <tr v-if="records && recordsData.length === 0" class="h-[60px] bg-white">
                        <td colspan="4" class="px-[24px] py-[16px] text-center text-black/60">No items found.</td>
                    </tr>
                    <template v-if="records && recordsData.length" v-for="(item, index) in recordsData" :key="item.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-light text-base text-left">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ item.name || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ item.type || '-' }}</td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(item.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(item.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                <div v-if="activeMenuIndex === index"
                                    ...
                                    <router-link :to="`/taxonomies/global/${typeParam}/${item.id}`"
                                    ...
                                    <button class="flex items-center gap-[10px] text-[#F12222]"
                                        @click="openDeletePopup(`/cms/taxonomy/${typeParam}/${item.id}`)">
...
const selectedType = computed({
    get: () => typeParam.value,
    set: (v) => { if (v) navigateTo(`/taxonomies/global/${v}`); }
});
...
onMounted(async () => {
    pagination.reset();
    await fetchRecords();
    pageTitle.setTitle(`Taxonomies: ${typeLabel.value}`);
    pageTitle.setBreadcrumbs(['Taxonomies', 'Global', typeLabel.value]);
...
const fetchRecords = async () => {
    try {
        const base = `/cms/taxonomy/${typeParam.value}`;
        const url = `${base}?page=${pagination.page}${keyword.value ? `&keyword=${keyword.value}` : ''}`;
        ...
        console.error('Taxonomies fetch error:', error);
        records.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load taxonomies.');
    }
};
...
const onSubmit = () => { fetchRecords(); };

watch(keyword, () => { fetchRecords(); });
watch(pagination.page, () => { fetchRecords(); });
watch(typeParam, () => { fetchRecords(); pageTitle.setTitle(`Taxonomies: ${typeLabel.value}`); pageTitle.setBreadcrumbs(['Taxonomies', 'Global', typeLabel.value]); });
</script>
