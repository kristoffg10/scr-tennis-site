    ...
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    Displaying {{ records.length }} of {{ totalRecords }} Office
                    Locations
                </div>
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Location' : 'Add a Location'">
            <Form
                @submit="onSubmit"
                ...
                <SelectField
                    class="col-span-2"
                    label="Location Type"
                    v-model="formInitialValues.locationType"
                    name="locationType"
                ...
                <TextField
                    label="Location Name"
                    v-model="formInitialValues.officeLocationName"
                ...
                <TextField
                    label="Address"
                    v-model="formInitialValues.address"
                    name="address"
                ...
                <TextField
                    label="Trunk Line Number"
                    v-model="formInitialValues.trunklineNumber"
                    name="trunklineNumber"
                    placeholder="Enter contact number"
                    :rules="'required'" />

                <TextField
                    label="Google Map URL"
                    v-model="formInitialValues.googleMapUrl"
                    name="googleMapUrl"
                    placeholder="Paste Google Map Link"
                    :rules="'required|googleMap'"
                    optionalMessage="Please paste the Google Map link here." />
...
onMounted(() => {
    pageTitle.reset();
    pageTitle.setTitle('Office Locations');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Contact Us', 'Office Locations']);
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/office-locations?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        console.log('doing');
...
const formInitialValues = computed(() => ({
    locationType: editingRecord.value?.location_type,
    officeLocationName: editingRecord.value?.name,
    address: editingRecord.value?.address,
    trunklineNumber: editingRecord.value?.contact_number,
    googleMapUrl: editingRecord.value?.map_url,
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        const payload = {
            name: values.officeLocationName,
            type: 'office_location',
            location_type: values.locationType,
            address: values.address,
            contact_number: values.trunklineNumber,
            map_url: values.googleMapUrl,
        };
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Office Location updated successfully!');
        } else {
            console.log(payload);
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Office Location added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.message ||
                'An error occurred while saving the Office Location'
        );
    } finally {
