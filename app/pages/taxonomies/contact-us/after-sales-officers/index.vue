    ...
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    Displaying {{ records.length }} of {{ totalRecords }} After
                    Sales Officers
                </div>
                <button
                    @click="openPopup()"
            ...
            <table class="w-full border-separate border-spacing-y-[12px]">
                ...
                <tbody>
                    <tr
                        ...
                        <td class="py-[34.5px]">
                            <span class="font-[500]">{{ record.email }} </span>
                        </td>
                        <td class="py-[34.5px]">
                            <span>
                                {{
        ...
        <PopupForm
            ...
            <Form
                ...
                <TextField
                    label="First Name"
                    name="firstName"
                    v-model="formInitialValues.firstName"
                    placeholder="Enter first name"
                    :rules="'required'" />
                <TextField
                    label="Last Name"
                    name="lastName"
                    v-model="formInitialValues.lastName"
                    placeholder="Enter last name"
                    :rules="'required'" />
                <SelectField
                    class="col-span-2"
                    label="Officer Type"
                    v-model="formInitialValues.officerType"
                    name="officerType"
                    placeholder="Select officer type"
                    :options="[
                        {
                            value: 'Horizontal Projects (House and Lot)',
                            label: 'Horizontal Projects (House and Lot)',
                        },
                        {
                            value: 'Vertical Projects (Condominium)',
                            label: 'Vertical Projects (Condominium)',
                        },
                        {
                            value: 'Regional Projects',
                ...
                <TextField
                    ...
                    v-model="formInitialValues.contactNumber"
                    name="contactNumber"
                    placeholder="Enter Contact Number"
                    :rules="'required'" />
                <TextField
                    label="Email"
                    v-model="formInitialValues.email"
                    name="email"
                    placeholder="Enter email"
                    :rules="'required|email'" />
                <div
                    class="flex gap-[24px] mt-[16px] ml-auto max-w-max col-span-2">
                    <button
                        @click="closePopup"
                        type="button">
                        Cancel
...
onMounted(() => {
    pageTitle.reset();
    pageTitle.setTitle('After Sales Officers');
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/after-sales-officers?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
...
const formInitialValues = computed(() => ({
    firstName: editingRecord.value?.first_name,
    lastName: editingRecord.value?.last_name,
    officerType: editingRecord.value?.officer_type,
    contactNumber: editingRecord.value?.contact_number,
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        const payload = {
            type: 'after_sales_officer',
            first_name: values.firstName,
            last_name: values.lastName,
            officer_type: values.officerType,
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('After Sales Officer updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('After Sales Officer added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.message ||
                'An error occurred while saving the After Sales Officer'
        );
    } finally {
