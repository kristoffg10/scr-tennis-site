<template>
    <div class="overflow-y-scroll">
        <div class="p-[16px]">
            ...
            <div class="flex items-center justify-between mb-[12px]">
                <div>
                    ...
                    {{ totalRecords }} Priority Locations
                </div>
                <button
                    @click="openPopup()"
                    class="main-button">
                    ...
                    <span> Add a Priority Location</span>
                </button>
            </div>
            <!-- Table  -->
        ...
        <PopupForm
            ...
            :title="isEditing ? 'Edit Priority Location' : 'Add a Priority Location'">
            <Form
                @submit="onSubmit"
                :initial-values="formInitialValues">
                <TextField
                    label="Priority Location"
                    v-model="formInitialValues.priorityLocation"
                    name="priorityLocation"
                    placeholder="Enter a Priority Location"
                    :rules="'required|max:70'"
                    optionalMessage="Max 70 characters" />
...
onMounted(() => {
    pageTitle.reset();
    pageTitle.setTitle('Priority Location');
    pageTitle.setBreadcrumbs(['Taxonomies', 'Sellers - Sales Group', 'Priority Locations']);
...
const fetchRecords = async (page = 1) => {
    try {
        const response = await nuxtApp.$axios.get(
            `/cms/taxonomies/priority-locations?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`
        );
        console.log('doing');
...
const formInitialValues = computed(() => ({
    priorityLocation: editingRecord.value?.name || '',
}));
...
const onSubmit = async (values, { resetForm }) => {
    ...
    try {
        const payload = {
            name: values.priorityLocation,
            type: 'priority_location',
        };
        ...
        if (isEditing.value) {
            ...
            nuxtApp.$toast.success('Priority Location updated successfully!');
        } else {
            await nuxtApp.$axios.post('/cms/taxonomies', payload);
            nuxtApp.$toast.success('Priority Location added successfully!');
        }
        ...
        nuxtApp.$toast.error(
            error.response?.data?.errors.name[0] ||
                'An error occurred while saving the Priority Location'
        );
    } finally {
