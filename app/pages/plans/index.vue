    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="plans && plans.data" class="text-black font-medium text-base">
                ...
                <div class="flex gap-[16px] items-center">
                    ...
                    <router-link :to="`/plans/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                ...
                <tbody>
                    <template v-if="plans && plans.data" v-for="(plan, index) in plans.data" :key="plan.id">
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ plan.title }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ plan.type }}</td> 
                            ...
                            <td class="px-[24px] py-[8px] ">
                                {{ $moment(plan.updated_at ? plan.updated_at : plan.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black"> <!-- Adjust styling as needed -->
                                    {{ $moment(plan.updated_at).format('hh:mm A') }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    ...
                                    <button
                                        ...
                                        @click="openDeletePopup(`/cms/plans/${plan.id}`)"
                                    >
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="18"
...
<script setup>
    ...
    onMounted( async () => {
        ...
        pageTitle.setTitle("Plans");
        pageTitle.setBreadcrumbs(['Plans', 'Plans List']);
    ...
    const fetchRecords = async () => {
        try {
            const plans_response = await nuxtApp.$axios.get(`/cms/plans?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            plans.value = plans_response.data.records;

            pagination.setTotalPages(plans_response.data.records.last_page);
        } catch (error) {
