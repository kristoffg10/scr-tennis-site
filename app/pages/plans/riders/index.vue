        ...
        <div v-if="selectedPlanId" class="p-[16px] gap-y-[20px] overflow-y-auto gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/plans/riders/${selectedPlanId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                ...
                <tbody>
                    <template v-if="records && records.data && records.data.length > 0">
                        <template v-for="(record, index) in records.data" :key="record.id">
                            <tr class="h-3">
                                <td colspan="4"></td>
                            </tr>
                            <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                                ...
                                <td class="px-[24px] py-[8px] relative w-[150px]">
                                    <div class="flex items-center gap-[8px]">
                                        <router-link
                                            :to="`/plans/riders/${selectedPlanId}/${record.id}`"
                                        ...
                                        <button
                                            ...
                                            @click="openDeletePopup(`/cms/plan-rider/${selectedPlanId}/${record.id}`)"
                                        >
                    ...
                    <tr v-else-if="records && records.data && records.data.length === 0">
                        <td colspan="4" class="px-[24px] py-[24px] text-center text-black/60">
                            No riders found for this plan.
                        </td>
                    </tr>
...
onMounted(async () => {
    pagination.reset();
    pageTitle.setTitle("Group Plan - Riders");
    pageTitle.setBreadcrumbs(['Plan Types', 'Group Plan', 'Riders']);
    await fetchPlans();
});
...
const fetchPlans = async () => {
    try {
        ...
        const allPlans = response.data.records || [];
        // Only show GROUP plans in this module
        plans.value = allPlans.filter(plan => plan.type === 'group');
...
const fetchRecords = async () => {
    ...
    try {
        const response = await nuxtApp.$axios.get(`/cms/plan-rider/${selectedPlanId.value}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`);
        ...
        } else if (records.value && Array.isArray(records.value)) {
            // Handle case where records is an array (when using ?all=1)
            records.value = {
                data: records.value,
