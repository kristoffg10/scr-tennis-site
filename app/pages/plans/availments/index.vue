        ...
        <div v-if="selectedPlanId" class="p-[16px] gap-y-[20px] overflow-y-auto gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/plans/availments/${selectedPlanId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
                ...
                <tbody>
                            ...
                            <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                                ...
                                <td class="px-[24px] py-[8px] relative w-[150px]">
                                    <div class="flex items-center gap-[8px]">
                                        <router-link
                                            :to="`/plans/availments/${selectedPlanId}/${record.id}`"
                                        ...
                                        <button
                                            ...
                                            @click="openDeletePopup(`/cms/plan-availment/${selectedPlanId}/${record.id}`)"
                                        >
                    ...
                    <tr v-else-if="records && records.data && records.data.length === 0">
                        <td colspan="4" class="px-[24px] py-[24px] text-center text-black/60">
                            No availments found for this plan.
                        </td>
                    </tr>
                </tbody>
            </table>
...
onMounted(async () => {
    pagination.reset();
    pageTitle.setTitle("Individual Plan - Availments");
    pageTitle.setBreadcrumbs(['Plan Types', 'Individual Plan', 'Availments']);
    await fetchPlans();
});
...
const fetchPlans = async () => {
    try {
        ...
        // Only show INDIVIDUAL plans in this module
        plans.value = allPlans.filter(plan => plan.type === 'individual');
...
const fetchRecords = async () => {
    ...
    try {
        const response = await nuxtApp.$axios.get(`/cms/plan-availment/${selectedPlanId.value}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}`);
