<template>
    <div class="relative w-full p-[16px] bg-offwhite flex flex-col gap-[16px] overflow-y-auto mb-[32px]">
        <Form v-if="recordData" :key="`form-${recordData?.id || 'new'}`" id="form" @submit="submit()" :initial-values="initialValues" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                <h2 class="font-semibold text-xl text-start w-full">Availment Information</h2>
                <div class="grid grid-cols-1 gap-[16px]">
                    <TextField label="Title" name="title" v-model="formData.title" placeholder="e.g. Hospitalization Coverage" :rules="'required|max:255'"
...
onMounted(() => {
    pageTitle.setTitle(`Edit Availment`);
    pageTitle.setBreadcrumbs(['Plans', 'Availments', 'Edit Availment']);
    pageTitle.setPageFrom('Availments');
    pageTitle.setPageFromRoute('/plans/availments');
    fetchRecords();
});

const fetchRecords = async () => {
    try {
        const response = await nuxtApp.$axios.get(`/cms/plan-availment/${planId}/${id}`);
...
const submit = async () => {
    ...
    try {
        const response = await nuxtApp.$axios.post(`/cms/plan-availment/${planId}/${id}`, form_data, {
            headers: {
            ...
            const record = response.data.record;
            populateData(record);
            nuxtApp.$toast.success('Availment updated successfully!');
        });

    } catch (error) {
        console.error('Error:', error);
        nuxtApp.$toast.error('Error updating availment. Please try again.');
    }
}
