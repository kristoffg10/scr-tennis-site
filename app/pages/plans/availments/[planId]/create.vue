        ...
        <Form id="form" @submit="submit()" class="w-full gap-[16px] flex flex-col">
            <div class="bg-white p-[16px] flex flex-col gap-[16px] rounded-[10px] w-full">
                ...
                <div class="grid grid-cols-1 gap-[16px]">
                    <TextField label="Title" name="title" placeholder="e.g. Hospitalization Coverage" :rules="'required|max:255'"
...
onMounted(() => {
    pageTitle.setTitle(`Add Availment`);
    pageTitle.setBreadcrumbs(['Plans', 'Availments', 'Add Availment']);
...
const submit = async () => {
    ...
    try {
        const response = await nuxtApp.$axios.post(`/cms/plan-availment/${planId}`, form_data, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        }).then((response) => {
            const record = response.data.record;
            nuxtApp.$toast.success('Availment created successfully!');
            useRouter().push(`/plans/availments`);
        });

    } catch (error) {
        console.error('Error:', error);
        nuxtApp.$toast.error('Error creating availment. Please try again.');
    }
}
</script>
