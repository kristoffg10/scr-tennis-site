<template>
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div v-if="parentId && parentId !== '8e9c9978-ce11-4e33-8ad6-fadbaa9b8995'" class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full">
                <label for="search" class="text-base font-medium text-black/70">
                    Search for Tab title
                </label>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            ...
            <div v-if="tabs === null" class="bg-yellow-50 border border-yellow-200 rounded p-4 text-sm">
                <p class="font-semibold">Debug Info:</p>
                <p>Parent ID: {{ parentId || 'Missing' }}</p>
                <p>Tabs value: {{ tabs === null ? 'null' : 'loaded' }}</p>
                <p>Route path: {{ route.path }}</p>
                <p>Route params: {{ JSON.stringify(route.params) }}</p>
            </div>
            
            <div class="flex justify-between items-center w-full">
                <p v-if="tabs && tabs.data && tabs.data.length > 0" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ tabs.from || 1 }} - {{ tabs.to || tabs.data.length }} 
                    <span class="text-black/60">of</span> 
                    {{ tabs.total || tabs.data.length }} 
                    <span class="text-black/60">items</span>
                </p>
                <p v-else-if="tabs && tabs.data && tabs.data.length === 0" class="text-black/60 font-medium text-base">
                    No tabs found
                </p>
                <div class="flex gap-[16px] items-center">
                    <router-link 
                        v-if="parentId && parentId !== 'undefined' && showAddTabButton" 
                        :to="`/section-tabs/${parentId}/create`" 
                        class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition hover:bg-dark-ui-color flex gap-[4px]">
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        <template v-if="!hideSeqAndDelete">
                ...
                <tbody>
                    <template v-if="tabs && tabs.data" v-for="(tab, index) in tabs.data" :key="tab.id">
                        <tr class="h-3"><td :colspan="hideSeqAndDelete ? 2 : 3"></td></tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td v-if="!hideSeqAndDelete" class="px-[24px] py-[8px] text-base font-normal">{{ tab.sequence }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ tab.title || '-' }}</td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    <router-link
                                        v-if="(parentId && parentId !== 'undefined') || tab.parent_id" 
                                        :to="`/section-tabs/${tab.parent_id || parentId}/${tab.id}`"
                                    ...
                                    <button 
                                        ...
                                        @click="openDeletePopup(`/cms/section-tab/${tab.parent_id || parentId}/${tab.id}`)">
                    ...
                    <tr v-else-if="tabs && tabs.data && tabs.data.length === 0">
                        <td :colspan="hideSeqAndDelete ? 2 : 3" class="px-[24px] py-[32px] text-center text-black/60">
                            <p class="text-base">No tabs found.</p>
                            <p class="text-sm mt-2">Click "Add Tab" to create your first tab.</p>
                        </td>
                    </tr>
                    <tr v-else-if="!tabs">
                        <td :colspan="hideSeqAndDelete ? 2 : 3" class="px-[24px] py-[32px] text-center text-black/60">
                            <p class="text-base">Loading tabs...</p>
                        </td>
                    </tr>
                </tbody>
            </table> 
...
<script setup>
    ...
    const parentId = computed(() => {
        ...
        let id = route.params.parent_id;
        
        // If not available from route, try to get from first tab's parent_id
        if ((!id || id === 'undefined') && tabs.value && tabs.value.data && tabs.value.data.length > 0) {
            id = tabs.value.data[0].parent_id;
            console.log('Parent ID retrieved from tab data:', id);
        }
        
        if (!id || id === 'undefined') {
            console.error('Parent ID is missing or undefined. Route params:', route.params);
            console.error('Tabs data:', tabs.value);
        }
        return id;
    });

    const hideAddTabForParentIds = [
    '1f458320-4ebb-4200-aecc-f7547e798948',
    '515a7602-82ed-47dc-8236-4e0e9ab3e57f',
    '8e9c9978-ce11-4e33-8ad6-fadbaa9b8995',
    '47b48c8d-ce83-4ad9-978f-15f1704f1513',
    ];

    const showAddTabButton = computed(() => {
        if (!parentId.value || parentId.value === 'undefined') return false;
        return !hideAddTabForParentIds.includes(parentId.value);
    });

    // For this specific parent ID, hide the sequence column and delete button
    const hideSeqAndDelete = computed(() => parentId.value === '544f0ae5-920b-43b1-912f-5955795e844c');

    onMounted( async () => {
        if (!parentId.value || parentId.value === 'undefined') {
            console.error('Parent ID is missing in route. Current route:', route.path);
            console.error('Route params:', route.params);
            nuxtApp.$toast?.error?.('Invalid section ID. Please navigate from a valid page section.');
            return;
        }
        pagination.reset();
        await fetchRecords();
        pageTitle.setTitle("Section Tabs");
        pageTitle.setBreadcrumbs(['Pages', 'Section Tabs']);
    ...
    const fetchRecords = async () => {
        try {
            // Get parent ID from route params first
            let sectionId = route.params.parent_id;
            
            // If not available, show error
            if (!sectionId || sectionId === 'undefined') {
                console.error('Parent ID is missing from route. Route path:', route.path);
                console.error('Route params:', route.params);
                nuxtApp.$toast?.error?.('Invalid section ID. Please navigate from a valid page section.');
                return;
            }
            
            const tabs_response = await nuxtApp.$axios.get(`/cms/section-tab/${sectionId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            
            console.log('Tabs API Response:', tabs_response);
            console.log('Tabs Response Data:', tabs_response.data);
            
            // Handle different response structures
            if (tabs_response.data.records) {
                tabs.value = tabs_response.data.records;
                
                // Ensure parent_id is set in tabs data if missing
                if (tabs.value && tabs.value.data) {
                    tabs.value.data.forEach(tab => {
                        if (!tab.parent_id) {
                            tab.parent_id = sectionId;
                        }
                    });
                }
                
                if (tabs.value.last_page) {
                    pagination.setTotalPages(tabs.value.last_page);
                }
            } else if (Array.isArray(tabs_response.data)) {
                // If response is a direct array, convert to paginated format
                tabs.value = {
                    data: tabs_response.data,
                    total: tabs_response.data.length,
                    from: 1,
                    to: tabs_response.data.length,
                    current_page: 1,
                    last_page: 1
                };
                ...
                console.error('Unexpected API response structure:', tabs_response.data);
                tabs.value = null;
            }
            
            console.log('Tabs value after processing:', tabs.value);
        } catch (error) {
            console.error('Error fetching tabs:', error);
            console.error('Error response:', error.response);
            console.error('Error data:', error.response?.data);
            tabs.value = null;
            nuxtApp.$toast?.error?.(error.response?.data?.message || 'Failed to load tabs. Please try again.');
        }
    };
    ...
    // Parent IDs for which the delete button is hidden (e.g. claim types 1f458320...)
    const hideDeleteForParentIds = ['515a7602-82ed-47dc-8236-4e0e9ab3e57f', '47b48c8d-ce83-4ad9-978f-15f1704f1513', '8e9c9978-ce11-4e33-8ad6-fadbaa9b8995', '1f458320-4ebb-4200-aecc-f7547e798948'];
    const showDeleteButton = (tab) => {
        const pid = tab?.parent_id || parentId.value;
        if (!pid || pid === 'undefined') return false;
        return !hideDeleteForParentIds.includes(pid);
    };
