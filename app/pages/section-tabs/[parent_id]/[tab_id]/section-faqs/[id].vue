...
onMounted(() => {
    isClient.value = true;
    pageTitle.setTitle(`Edit Tab FAQ`);
    pageTitle.setBreadcrumbs(['Tab FAQs', 'Edit FAQ']);
    pageTitle.setPageFrom('Tab FAQs');
    pageTitle.setPageFromRoute(`/section-tabs/${parentId}/${tabId}/section-faqs`);
    fetchRecords();
});

const fetchRecords = async () => {
    try {
        const faq_response = await nuxtApp.$axios.get(`/cms/section-tab/${parentId}/${tabId}/section-faq/${id}`);
...
const submit = async () => {
    ...
    try {
        await nuxtApp.$axios.post(`/cms/section-tab/${parentId}/${tabId}/section-faq/${id}`, form_data, {
            headers: {
...
const populateData = (data) => {
    ...
    // Ensure sequence is always at least 1 for UX, matching backend rules
