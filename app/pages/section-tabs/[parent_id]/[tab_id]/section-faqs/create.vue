...
onMounted(() => {
    isClient.value = true;
    pageTitle.setTitle(`Create Tab FAQ`);
    pageTitle.setBreadcrumbs(['Tab FAQs', 'Create FAQ']);
...
const submit = async () => {
    ...
    try {
        await nuxtApp.$axios.post(`/cms/section-tab/${parentId}/${tabId}/section-faq`, form_data, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        }).then((response) => {
            nuxtApp.$toast.success('FAQ created successfully!');
            router.push(`/section-tabs/${parentId}/${tabId}/section-faqs`);
        });
    } catch (error) {
