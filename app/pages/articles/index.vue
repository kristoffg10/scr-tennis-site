    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        <div class="relative w-full flex p-[16px] justify-between items-center bg-white">
            <form @submit="onSubmit" class="gap-y-[8px] w-full flex flex-col">
                <div class="flex gap-[16px] items-end">
                    ...
                    <div class="w-[200px]">
                        <label for="category" class="text-base font-medium text-black/70">
                            Filter by Category
                        </label>
                        <select 
                            id="category"
                            ...
                            <option 
                                v-for="option in categoryOptions.filter(opt => opt.value !== '')" 
                                :key="option.value" 
                                :value="option.value">
                                {{ option.label }}
                            </option>
                    ...
                    <div class="w-[160px]">
                        <label for="featured" class="text-base font-medium text-black/70">
                            Filter by Featured
                        </label>
                        <select 
                            id="featured"
                    ...
                    <div class="w-[160px]">
                        <label for="enabled" class="text-base font-medium text-black/70">
                            Filter by Enabled
                        </label>
                        <select 
                            id="enabled"
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="articles" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ articles.from ?? 0 }} - {{ articles.to ?? 0 }} 
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Date Published</th>
                        <th class="px-[24px] py-[8px]">Featured</th>
                        <th class="px-[24px] py-[8px]">Enabled</th>
                        <th class="px-[24px] py-[8px]">Date Edited</th>
                        <th></th>
                    </tr>
                </thead>
                <tbody>
                    <tr v-if="articles && articlesData.length === 0" class="h-[60px] bg-white">
                        <td colspan="8" class="px-[24px] py-[16px] text-center text-black/60">No articles found.</td>
                    </tr>
                    <template v-if="articles && articlesData.length" v-for="(article, index) in articlesData" :key="article.id">
                        <tr class="h-3">
                            <td colspan="8"></td>
                        </tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            ...
                            <td class="px-[24px] py-[8px] text-base font-semibold capitalize">{{ article.type || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ article.category?.name || '-' }}</td>
                            ...
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(article.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(article.updated_at).format('hh:mm A') }}</span>
                            </td>
...
const articlesData = computed(() => {
    ...
    if (Array.isArray(articles.value)) return articles.value;
    if (articles.value && Array.isArray(articles.value.data)) return articles.value.data;
    return [];
});
...
onMounted(async () => {
    ...
    pageTitle.setBreadcrumbs(['Articles', 'Articles List']);
...
const fetchRecords = async () => {
    try {
        ...
        const featuredParam = selectedFeatured.value !== '' ? `&featured=${selectedFeatured.value}` : '';
        const enabledParam = selectedEnabled.value !== '' ? `&enabled=${selectedEnabled.value}` : '';
        const response = await nuxtApp.$axios.get(`/cms/articles?page=${pagination.page}&keyword=${keyword.value || ''}${typeParam}${categoryParam}${featuredParam}${enabledParam}`);
        const rec = response.data?.records;
        // Paginator: { data, from, to, total, last_page }; or array when all=1
        if (Array.isArray(rec)) {
            articles.value = { data: rec, from: 1, to: rec.length, total: rec.length, last_page: 1 };
        } else if (rec && typeof rec === 'object' && Array.isArray(rec.data)) {
            articles.value = rec;
        } else {
            articles.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        }
        if (articles.value?.last_page) {
            pagination.setTotalPages(articles.value.last_page);
        }
    } catch (error) {
        console.error('Articles fetch error:', error);
        articles.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load articles. Check the API and try again.');
    }
};
