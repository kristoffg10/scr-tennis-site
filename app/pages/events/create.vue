    ...
    <div class="relative z-10 max-w-4xl mx-auto px-4 lg:px-8 flex flex-col gap-8">
      ...
      <header class="flex flex-col gap-1">
        ...
        <h1 class="text-3xl md:text-4xl font-semibold text-white leading-tight">
          Add <span class="text-[#D4AF37]">Event</span>
        </h1>
        <p class="text-sm text-white/50 mt-1">Fill in the details below to create a new club event.</p>
      </header>
      ...
      <Form id="form" @submit="submit()" class="flex flex-col gap-6">
        ...
        <div class="rounded-2xl border border-[#C9A227]/20 bg-white/5 backdrop-blur-sm overflow-hidden">
          ...
          <div class="p-6 flex flex-col gap-5">
            ...
            <!-- Doubles / Mixed: two pairs of players, Winner = Team 1 or Team 2 -->
        ...
        <div class="rounded-2xl border border-[#C9A227]/20 bg-white/5 backdrop-blur-sm overflow-hidden">
          ...
          <div class="px-6 py-5 border-b border-white/10 flex items-center gap-3">
            ...
            <!-- Badge -->
            <span class="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full bg-[#C9A227]/15 text-[#D4AF37] border border-[#C9A227]/20">
...
onMounted(() => {
  pageTitle.setTitle('Add Event');
  pageTitle.setBreadcrumbs(['Club Events', 'Events List', 'Add Event']);
...
const submit = async () => {
  ...
  try {
    const response = await nuxtApp.$axios.post('/cms/events', form_data, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    nuxtApp.$toast.success('Event created successfully!');
    useRouter().push(`/events/${response.data.record.id}`);
  } catch (error) {
    console.error('Error:', error);
    nuxtApp.$toast.error(error?.response?.data?.message || 'Error creating event. Please try again.');
  }
};
</script>
