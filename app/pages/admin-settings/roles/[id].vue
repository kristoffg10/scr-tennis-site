<template>
  ...
  // ─── Inline sub-component: PermissionTile ──────────────────────────────────────
  // Keeps things in one file; move to its own .vue if preferred.
  const PermissionTile = defineComponent({
    props: {
    ...
    emits: ['toggle'],
    setup(props, { emit }) {
      // Generate a stable emoji/icon based on the unique key so each tile
      // looks distinct without needing a full icon library.
      const iconMap = {
        'pages-': '📄',
        'articles': '📰',
        'agents': '👤',
      ...
      const icon = computed(() => {
        const u = props.option.unique || '';
        for (const [key, val] of Object.entries(iconMap)) {
          if (u.startsWith(key) || u === key) return val;
        }
        return '📌';
      });
      ...
      return () => h(
        'button',
        {
          ...
          onClick: () => emit('toggle'),
          class: [
            'relative group flex items-start gap-3 p-3 rounded-[10px] border-2 text-left cursor-pointer transition-all duration-150 w-full',
            props.selected
        ...
        [
          ...
          h('span', {
            class: [
              'shrink-0 mt-0.5 w-5 h-5 rounded-[5px] border-2 flex items-center justify-center transition-colors',
              props.selected ? 'bg-ui-color border-ui-color' : 'border-gray-300 group-hover:border-gray-400',
            ].join(' '),
          }, [
            props.selected
              ? h('svg', {
          ...
          h('span', { class: 'flex flex-col min-w-0' }, [
            h('span', { class: 'flex items-center gap-1.5' }, [
              h('span', { class: 'text-base leading-none' }, icon.value),
              h('span', {
                class: [
                  'text-sm font-medium leading-snug truncate',
                  props.selected ? 'text-ui-color' : 'text-gray-700',
                ].join(' '),
  ...
  // Group definitions with icons (inline SVG strings for v-html)
  const GROUP_CONFIG = {
    pages: {
      label: 'Page Modules',
      order: 1,
  ...
  const populateData = (data) => {
    ...
    selectedItems.value = Array.isArray(p) ? p : (typeof p === 'string' ? JSON.parse(p || '[]') : []);
  };
  
  const roleData2 = ref(null); // alias to satisfy Form v-if
  const roles = ref([]);
  </script>