    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="videos && videos.data" class="text-black font-medium text-base">
                ...
                <div class="hidden">
                    <router-link :to="`/section-videos/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">YouTube Title</th>
                        <th class="px-[24px] py-[8px]">YouTube ID</th>
                        <th class="px-[24px] py-[8px] flex gap-[8px] cursor-pointer"  @click="toggleSort('yt_published_date')">
                            Published Date
                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
                                <path d="M5.22216 7.34717L8.57972 3.1633C8.87841 2.7763 9.37059 2.7763 9.66985 3.1633L13.0274 7.34717C13.3261 7.73417 13.1855 8.0503 12.6758 8.0503H5.57428C5.06466 8.0503 4.92403 7.73361 5.22272 7.34717H5.22216Z"
                                    :fill="sortBy === 'yt_published_date' && sortDirection === 'asc' ? '#4B545B' : '#A6A8AB'" />
                                <path d="M13.0268 10.6519L9.66928 14.8357C9.3706 15.2227 8.87841 15.2227 8.57916 14.8357L5.2216 10.6519C4.92291 10.2649 5.06353 9.94873 5.57316 9.94873H12.6747C13.1843 9.94873 13.325 10.2654 13.0263 10.6519H13.0268Z"
                                    :fill="sortBy === 'yt_published_date' && sortDirection === 'desc' ? '#4B545B' : '#A6A8AB'" />
                            </svg>
                        </th>
                        <th></th>
                    </tr>
                ...
                <tbody>
                    <template v-if="videos && videos.data" v-for="(video, index) in videos.data" :key="video.id">
                        <tr class="h-3"><td colspan="5"></td></tr>
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ video.title || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ video.yt_title ? (video.yt_title.length > 40 ? video.yt_title.substring(0, 40) + '...' : video.yt_title) : '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-normal">{{ video.yt_id || '-' }}</td>
                            <td class="px-[24px] py-[8px]">
                                {{ video.yt_published_date ? $moment(video.yt_published_date).format('MMM DD, YYYY') : '-' }}
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                <div v-if="activeMenuIndex === index" class="absolute bottom-0 left-0 bg-white flex flex-col p-[10px] gap-[10px] rounded-[10px] translate-y-[70%] font-[600] border border-gray z-50">
                                    <router-link :to="`/section-videos/${parentId}/${video.id}`" class="flex items-center gap-[10px] pb-[10px] border-b border-black/30">
                                    ...
                                    <button class="hidden" @click="openDeletePopup(`/cms/section-video/${parentId}/${video.id}`)">
...
<script setup>
    ...
    onMounted( async () => {
        pagination.reset();
        await fetchRecords();
        pageTitle.setTitle("Section Videos");
        pageTitle.setBreadcrumbs(['Pages', 'Section Videos']);
    ...
    const fetchRecords = async () => {
        try {
            const videos_response = await nuxtApp.$axios.get(`/cms/section-video/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            videos.value = videos_response.data.records;
            pagination.setTotalPages(videos_response.data.records.last_page);
        } catch (error) {
            console.error('Error:', error);
        }
    };

    const sortBy = ref('yt_published_date');
