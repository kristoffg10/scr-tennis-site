    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
                ...
                <div class="flex gap-[16px] items-end">
                    <div class="flex-1">
                        <label for="search" class="text-base font-medium text-black/70">
                            Search for Annual Report title
                        </label>
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="reports" class="text-black font-medium text-base">
                ...
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/annual-reports/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                ...
                <tbody>
                    <tr v-if="reports && reportsData.length === 0" class="h-[60px] bg-white">
                        <td colspan="5" class="px-[24px] py-[16px] text-center text-black/60">No annual reports found.</td>
                    </tr>
                    <template v-if="reports && reportsData.length" v-for="(report, index) in reportsData" :key="report.id">
                        <tr class="h-3">
                            <td colspan="5"></td>
                        </tr>
                        <tr class="h-[40px] bg-white text-black text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ report.title || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ report.subtitle || '-' }}</td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">
                                <span
                                    class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium"
                                    :class="report.featured
                                        ? 'bg-ui-color/10 text-ui-color border border-ui-color/40'
                                        : 'bg-gray-100 text-gray-600 border border-gray-300'">
                                    {{ report.featured ? 'Yes' : 'No' }}
                                </span>
                            </td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(report.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(report.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                <div class="flex items-center gap-[8px]">
                                    <router-link
                                        :to="`/annual-reports/${report.id}`"
                                    ...
                                    <button
                                        ...
                                        @click="openDeletePopup(`/cms/annual-reports/${report.id}`)">
                                        <svg
                                            xmlns="http://www.w3.org/2000/svg"
                                            width="18"
                                            height="20"
...
const reportsData = computed(() => {
    ...
    if (Array.isArray(reports.value)) return reports.value;
    if (reports.value && Array.isArray(reports.value.data)) return reports.value.data;
    return [];
});
...
onMounted(async () => {
    pagination.reset();
    await fetchRecords();
    pageTitle.setTitle("Annual Reports");
    pageTitle.setBreadcrumbs(['Annual Reports', 'Annual Reports List']);
...
const fetchRecords = async () => {
    try {
        const response = await nuxtApp.$axios.get(`/cms/annual-reports?page=${pagination.page}&keyword=${keyword.value || ''}`);
        const rec = response.data?.records;
        if (Array.isArray(rec)) {
            reports.value = { data: rec, from: 1, to: rec.length, total: rec.length, last_page: 1 };
        } else if (rec && typeof rec === 'object' && Array.isArray(rec.data)) {
            reports.value = rec;
        } else {
            reports.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        }
        if (reports.value?.last_page) {
            pagination.setTotalPages(reports.value.last_page);
        }
    } catch (error) {
        console.error('Annual reports fetch error:', error);
        reports.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
        nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load annual reports. Check the API and try again.');
    }
};
