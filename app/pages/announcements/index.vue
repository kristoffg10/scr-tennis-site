<template>
      ...
      <div class="relative z-10 max-w-6xl mx-auto px-4 lg:px-8 flex flex-col gap-8">
        ...
        <header class="flex flex-col gap-1">
          ...
          <h1 class="text-3xl md:text-4xl font-semibold text-white leading-tight">
            Club <span class="text-[#D4AF37]">Announcements</span>
          </h1>
          <p class="text-sm text-white/50 mt-1">Publish and manage club news, notices, and updates.</p>
        </header>
        ...
        <div class="rounded-2xl border border-[#C9A227]/20 bg-white/5 backdrop-blur-sm overflow-hidden">
          ...
          <!-- Mobile card list -->
          <div v-if="announcementsData.length" class="md:hidden divide-y divide-white/5">
            <div
            ...
            >
              <div class="flex items-start justify-between gap-3">
                ...
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    <p class="text-sm font-semibold text-white/90">{{ announcement.title || '—' }}</p>
              ...
              <div class="flex flex-wrap items-center gap-3 text-xs">
                ...
                >
                  <span class="w-1.5 h-1.5 rounded-full" :class="announcement.enabled ? 'bg-emerald-400' : 'bg-white/30'" />
                  {{ announcement.enabled ? 'Enabled' : 'Disabled' }}
                </span>
                <span class="text-white/35">
                  Edited {{ $moment(announcement.updated_at).format('MMM DD, YYYY · hh:mm A') }}
                </span>
              </div>
            </div>
          </div>
            ...
            <table class="min-w-full">
              <thead>
                <tr class="border-b border-white/10">
                  ...
                  <th class="px-6 py-4 text-left text-xs font-semibold uppercase tracking-wider text-white/35">Status</th>
              ...
              <tbody v-if="announcementsData.length">
                <tr
                  v-for="announcement in announcementsData"
                  :key="announcement.id"
                ...
                >
                  ...
                  <td class="px-6 py-4">
                    <div class="flex items-center gap-3">
                      ...
                      <span class="text-sm font-semibold text-white/90">{{ announcement.title || '—' }}</span>
                    </div>
                  </td>
  
                  <!-- Date -->
                  <td class="px-6 py-4 text-sm text-white/55">
                  ...
                  <td class="px-6 py-4 text-xs text-white/55 leading-relaxed">
                    {{ $moment(announcement.updated_at).format('MMM DD, YYYY') }}<br />
          ...
          <!-- Empty state -->
          <div v-if="!announcementsData.length" class="px-6 py-16 text-center">
            ...
            <p class="text-white/30 text-sm">No announcements found.</p>
          </div>
        </div>
  ...
  const announcementsData = computed(() => {
    ...
    if (Array.isArray(announcements.value)) return announcements.value;
    if (Array.isArray(announcements.value.data)) return announcements.value.data;
    return [];
  });
  
  const fetchRecords = async () => {
    try {
      const response = await nuxtApp.$axios.get(`/cms/announcements?page=${pagination.page}&keyword=${keyword.value || ''}`);
      const rec = response.data?.records;
      if (Array.isArray(rec)) {
        announcements.value = { data: rec, from: 1, to: rec.length, total: rec.length, last_page: 1 };
      } else if (rec && typeof rec === 'object' && Array.isArray(rec.data)) {
        announcements.value = rec;
      } else {
        announcements.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
      }
      if (announcements.value?.last_page) {
        pagination.setTotalPages(announcements.value.last_page);
      }
    } catch (error) {
      console.error('Announcements fetch error:', error);
      announcements.value = { data: [], from: 0, to: 0, total: 0, last_page: 1 };
      nuxtApp.$toast?.error(error?.response?.data?.message || 'Failed to load announcements.');
    }
  };
  ...
  onMounted(async () => {
    ...
    pageTitle.setBreadcrumbs(['Announcements', 'Announcements List']);
