      ...
      <header class="flex items-start justify-between gap-4">
        <div class="flex flex-col gap-1">
          ...
          <h1 class="text-3xl md:text-4xl font-semibold text-white leading-tight">
            Edit <span class="text-[#D4AF37]">Announcement</span>
          </h1>
          <p class="text-sm text-white/50 mt-1">Update the details for this announcement.</p>
        </div>

        <!-- Status badge (populated after fetch) -->
        <div class="shrink-0 mt-1 flex items-center gap-2">
          <span
            v-if="announcementData?.enabled !== undefined"
            class="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border capitalize"
            :class="announcementData.enabled ? 'bg-[#C9A227]/15 text-[#D4AF37] border-[#C9A227]/25' : 'bg-white/10 text-white/80 border-white/15'"
          >
            <span class="w-1.5 h-1.5 rounded-full" :class="announcementData.enabled ? 'bg-[#D4AF37]' : 'bg-white/50'" />
            {{ announcementData.enabled ? 'Enabled' : 'Disabled' }}
          </span>
        </div>
      </header>
      ...
      <template v-if="!announcementData">
        <div class="rounded-2xl border border-[#C9A227]/20 bg-white/5 backdrop-blur-sm overflow-hidden animate-pulse">
          ...
          <div class="p-6 flex flex-col gap-4">
            ...
            <div class="h-6 w-24 bg-white/10 rounded-lg" />
          </div>
        </div>
      </template>

      <!-- ── Form (shown once data is loaded) ── -->
      <Form v-if="announcementData" id="form" @submit="submit()" class="flex flex-col gap-6">
