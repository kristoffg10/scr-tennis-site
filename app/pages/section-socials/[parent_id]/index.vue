    ...
    <div class="relative w-full flex flex-col gap-y-[15px] bg-offwhite overflow-y-auto">
        ...
        <div class="p-[16px] gap-y-[20px] overflow-y-auto  gap-[16px] flex flex-col">
            <div class="flex justify-between items-center w-full">
                <p v-if="socials && socials.data" class="text-black font-medium text-base">
                    <span class="text-black/60">Displaying </span> 
                    {{ socials.from }} - {{ socials.to }} 
                    <span class="text-black/60">of</span> 
                    {{ socials.total }} 
                    <span class="text-black/60">items</span>
                </p>
                <div class="flex gap-[16px] items-center">
                    <router-link :to="`/section-socials/${parentId}/create`" class="px-[16px] py-[10px] rounded-[10px] bg-ui-color border border-ui-color text-white text-base font-medium transition
            ...
            <table class="min-w-full border-collapse rounded-[10px] overflow-hidden bg-off-white border-separate border-spacing-0 relative z-10 pb-[140px]">
                <thead>
                    <tr class="h-[40px] bg-white text-black/60 font-medium text-base text-left">
                        ...
                        <th class="px-[24px] py-[8px]">Link</th>
                ...
                <Draggable
                    v-if="socials && socials.data"
                    v-model="socials.data"
                    item-key="id"
                    handle=".drag-handle"
                        ...
                        <tr class="h-[40px] bg-white text-black font-normal text-base text-left font-light">
                            <td class="px-[24px] py-[8px] text-base font-normal">
                                <div class="flex items-center gap-[8px]">
                                    ...
                                    <span>{{ social.sequence }}</span>
                                </div>
                            </td>
                            <td class="px-[24px] py-[8px] text-base font-semibold">{{ social.title || '-' }}</td>     
                            <td class="px-[24px] py-[8px] text-base font-normal">
                                {{ social.link ? (social.link.length > 50 ? social.link.substring(0, 50) + '...' : social.link) : '-' }}
                            </td>
                            <td class="px-[24px] py-[8px]">
                                {{ $moment(social.updated_at).format('MMM DD, YYYY') }}<br>
                                <span class="text-sm text-black">{{ $moment(social.updated_at).format('hh:mm A') }}</span>
                            </td>
                            <td class="px-[24px] py-[8px] relative w-[150px]">
                                ...
                                <div v-if="activeMenuIndex === index" class="absolute bottom-0 left-0 bg-white flex flex-col p-[10px] gap-[10px] rounded-[10px] translate-y-[70%] font-[600] border border-gray z-50">
                                    <router-link :to="`/section-socials/${parentId}/${social.id}`" class="flex items-center gap-[10px] pb-[10px] border-b border-black/30">
                                    ...
                                    <button class="flex items-center gap-[10px] text-[#F12222]" @click="openDeletePopup(`/cms/section-social/${parentId}/${social.id}`)">
...
<script setup>
    ...
    onMounted( async () => {
        pagination.reset();
        await fetchRecords();
        pageTitle.setTitle("Section Social Media Links");
        pageTitle.setBreadcrumbs(['Pages', 'Section Social Media Links']);
    ...
    const fetchRecords = async () => {
        try {
            const socials_response = await nuxtApp.$axios.get(`/cms/section-social/${parentId}?page=${pagination.page}&sortBy=${sortBy.value}&sortDirection=${sortDirection.value}&keyword=${keyword.value}`); 
            socials.value = socials_response.data.records;
            pagination.setTotalPages(socials_response.data.records.last_page);
        } catch (error) {
    ...
    const onReorder = async (newOrder) => {
        try {
            ...
            if (nuxtApp.$toast && nuxtApp.$toast.success) {
                nuxtApp.$toast.success('Social links reordered successfully!');
            }
        } catch (error) {
            console.error('Error reordering social links:', error);
            if (nuxtApp.$toast && nuxtApp.$toast.error) {
                nuxtApp.$toast.error('Failed to reorder social links.');
            }
        }
    };
